...
export const adminRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/imports/:id/apply", async (request, reply) => {
    ...
    const applied = await db.transaction(async (tx) => {
      ...
      for (const row of rows) await tx.insert(tickets).values({ eventId: job.eventId, orderId: row.orderId, ticketCode: row.ticketCode, ticketCodeHmac: ticketHmac(row.ticketCode), participantName: row.participantName, whatsapp: row.whatsapp, email: row.email, amountRupiah: row.amountRupiah, paymentStatus: row.paymentStatus, registeredAt: row.registeredAt, sourceRowNumber: row.sourceRowNumber, sourceImportId: id, isPresentInLatestImport: true }).onConflictDoUpdate({ target: [tickets.eventId, tickets.ticketCodeHmac], set: { orderId: row.orderId, ticketCode: row.ticketCode, participantName: row.participantName, whatsapp: row.whatsapp, email: row.email, amountRupiah: row.amountRupiah, paymentStatus: row.paymentStatus, registeredAt: row.registeredAt, sourceRowNumber: row.sourceRowNumber, sourceImportId: id, isPresentInLatestImport: true, updatedAt: new Date() } });
