...
export function ScannerPage() {
  ...
  const startCamera = async () => { if (!gateId) return; stopCamera(); try { const mediaStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: { ideal: "environment" }, width: { ideal: 1280, max: 1280 }, height: { ideal: 720, max: 720 } }, audio: false }); if (!mounted.current || !video.current) { mediaStream.getTracks().forEach((track) => track.stop()); return; } stream.current = mediaStream; video.current.srcObject = mediaStream; await video.current.play(); if (!mounted.current) { stopCamera(); return; } setCameraState("active"); const { BrowserMultiFormatReader, BarcodeFormat } = await import("@zxing/browser"); if (!mounted.current || !video.current || !stream.current) return; const hints = new Map(); hints.set(DecodeHintType.TRY_HARDER, true); hints.set(DecodeHintType.POSSIBLE_FORMATS, [BarcodeFormat.QR_CODE, BarcodeFormat.CODE_128, BarcodeFormat.CODE_39, BarcodeFormat.DATA_MATRIX]); const reader = new BrowserMultiFormatReader(hints, { delayBetweenScanSuccess: 700 }); const scanCanvas = document.createElement("canvas"); const scanCtx = scanCanvas.getContext("2d", { willReadFrequently: true }); const tick = async () => { if (!mounted.current || !stream.current || !video.current) return; if (!busy.current && scanCtx) { const vw = video.current.videoWidth, vh = video.current.videoHeight; if (vw && vh) { const s = Math.min(1, 640 / vw); scanCanvas.width = Math.max(1, Math.round(vw * s)); scanCanvas.height = Math.max(1, Math.round(vh * s)); scanCtx.drawImage(video.current, 0, 0, scanCanvas.width, scanCanvas.height); try { const res = await reader.decodeFromCanvas(scanCanvas); const code = res?.getText(); if (code && code !== lastCode.current) { lastCode.current = code; void submit(code); } } catch { /* no code found this frame */ } } } detectionTimer.current = window.setTimeout(() => void tick(), 120); }; void tick(); if (!mounted.current) stopCamera(); } catch { stopCamera(); if (mounted.current) setCameraState("denied"); } };
  useEffect(() => { let buffer = ""; let timer = 0; const listener = (event: globalThis.KeyboardEvent) => { if ((event.target as HTMLElement).matches("input, textarea, select")) return; if (event.key === "Enter" && buffer) { void submit(buffer); buffer = ""; return; } if (event.key.length === 1) { buffer += event.key; clearTimeout(timer); timer = window.setTimeout(() => { buffer = ""; }, 1000); } }; addEventListener("keydown", listener); return () => removeEventListener("keydown", listener); }, [submit]);
